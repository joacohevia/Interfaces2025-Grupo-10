...
const regexLetters = /^[a-zA-Z\s]*$/ //acepta solo letras y espacios
const regexNumLet = /^[a-zA-Z0-9\s]*$/ //acepta letras, numeros y espacios
const regexMail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/ //acepta formato email
...
function validateInfo(input, message, regex, errorText) {
  ...
  if (!regex.test(value) || value === "") {//si no cumple con la regla o esta vacio
    if (message) {
...
function validateAge(input, message) {
  ...
  if (isNaN(age) || age < 10 || age > 95 || input.value === "") {
    showErrorMessage(message, "Ingrese una edad válida entre 10 y 95 años")
    addError(input)
    return false
  } else {
    hideErrorMessage(message)
