...
async function loadGames() {
    try {
        ...
        // Fallback al archivo local si la API falla
        try {
            const fallbackResponse = await fetch('../assets/API.json');
            if (fallbackResponse.ok) {
                const games = await fallbackResponse.json();
                console.log('Juegos cargados desde archivo local de fallback');
                window.currentGamesData = games;
                displayGames(games);
                return;
            }
        } catch (fallbackError) {
            console.error('Error cargando archivo de fallback:', fallbackError);
        }
