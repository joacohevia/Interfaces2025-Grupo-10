...
function displayGames(games) {
    ...
    // Set para trackear juegos ya asignados
    const usedGameIds = new Set();

    // Función auxiliar para filtrar juegos no usados
    function filterUnusedGames(gamesList) {
        return gamesList.filter(game => !usedGameIds.has(game.id));
    }

    // Función para marcar juegos como usados
    function markGamesAsUsed(gamesList) {
        gamesList.forEach(game => usedGameIds.add(game.id));
    }
    
    // Categorías de juegos basadas en géneros y características
    const categories = {};

    // Primero filtramos los juegos de lógica
    const logicGamesList = games.filter(game => {
    ...
    categories.logicGames = logicGamesList.slice(0, 3);
    markGamesAsUsed(categories.logicGames);

    // Luego los juegos sugeridos (mejores ratings)
    const suggestedGamesList = filterUnusedGames(
        games.filter(game => game.rating && game.rating >= 4.0)
            .sort((a, b) => b.rating - a.rating)
    );
    categories.suggestedGames = suggestedGamesList.slice(0, 3);
    markGamesAsUsed(categories.suggestedGames);

    // Juegos clásicos
    const classicGamesList = filterUnusedGames(
        games.filter(game => {
    ...
    categories.classicGames = classicGamesList.slice(0, 3);
    markGamesAsUsed(categories.classicGames);

    // Juegos de cocina
    const cookingGamesList = filterUnusedGames(
        games.filter(game => {
    ...
    categories.cookingGames = cookingGamesList.slice(0, 3);
    markGamesAsUsed(categories.cookingGames);

    // Juegos de deportes
    const sportsGamesList = filterUnusedGames(
        games.filter(game => {
            if (!game.genres) return false;
            const name = game.name.toLowerCase();
            return game.genres.some(genre => {
    ...
    categories.sportsGames = sportsGamesList.slice(0, 3);
    markGamesAsUsed(categories.sportsGames);
    };
...
function filterGames(searchTerm) {
    const gameCards = document.querySelectorAll('.game-card-small');
...
function showLoadingState() {
    const categories = ['logicGames', 'suggestedGames', 'classicGames', 'cookingGames', 'sportsGames'];
