...
// Referencias DOM
const btnIniciar = document.getElementById('startGame');
...
if (btnReiniciar) {
  btnReiniciar.addEventListener('click', () => {
    if (indiceNivelActual !== null) {
      ...
      mezclarPiezas();
      contadorCorrectas = 0;
      // Ocultar el botón de siguiente nivel para que no se pueda avanzar
      ...
      const winOverlay = document.getElementById('victoria-final');
      if (winOverlay) winOverlay.remove();
      // Ocultar overlay de derrota por tiempo si existe (canvas overlay)
      ctx.clearRect(0, 0, ANCHO_CANVAS, ALTO_CANVAS);
      // Si existe overlay de win del último nivel (div), eliminarlo
      const winDiv = document.getElementById('victoria-final');
      if (winDiv) winDiv.remove();
      // Resetear el estado para bloquear avance
      juegoEnCurso = false;
      detenerTemporizador();
      tiempoInicio = Date.now();
      iniciarTemporizador();
