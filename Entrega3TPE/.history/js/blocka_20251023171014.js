...
function showStartButton() {
  ...
  const boardSim = document.createElement('div');
...
function setSubdivisions(subdiv) {
  ...
  // Crear estructura igual a juego.html pero con posiciones ajustadas
  const boardSim = document.createElement('div');
  ...
  portadaImg.src = 'assets/img/pegEjecucuion.png';
  portadaImg.alt = '';
  boardSim.appendChild(portadaImg);
  ...
  // Share button y dropdown (abajo a la izquierda)
  const shareContainer = document.createElement('div');
  shareContainer.className = 'share-container';
  shareContainer.style.position = 'absolute';
  shareContainer.style.left = '24px';
  shareContainer.style.bottom = '24px';
  ...
  // Expand button (abajo a la derecha)
  const expandBtn = document.createElement('button');
  expandBtn.className = 'overlay-btn expand-btn';
  expandBtn.style.position = 'absolute';
  expandBtn.style.right = '24px';
  expandBtn.style.bottom = '24px';
  expandBtn.style.zIndex = '10';
  expandBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg>`;

  boardSim.appendChild(startBtn);
  boardSim.appendChild(shareContainer);
  boardSim.appendChild(expandBtn);
  gameDisplayContainer.appendChild(boardSim);

  // Funcionalidad dropdown share
