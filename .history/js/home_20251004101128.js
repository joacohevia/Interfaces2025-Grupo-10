...
function displayGames(games) {
    ...
    const categories = {
        logicGames: games.filter(game => {
            console.log('Checking game:', game.name, 'genres:', game.genres);
            if (!game.genres) return false;
            const result = game.genres.some(genre => {
                const genreName = genre.name.toLowerCase();
                const matches = genreName.includes('puzzle') || 
                ...
                if (matches) console.log('Match found for:', game.name, 'genre:', genreName);
                return matches;
            });
            return result;
        }).slice(0, 4),
